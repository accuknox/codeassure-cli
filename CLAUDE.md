...
- **Tools** (`tools.py`): `read_file` and `grep_code`, sandboxed to the codebase and further to an `anchor_root` derived per finding. Every read records line ranges into `AnalyzerDeps.accessed_paths`; verdict `evidence_locations` are then validated against what was actually read (`_validate_evidence*`) — the model can't cite code it never saw.
...
- **Optional extra layers**, all off by default: evaluator pass (`_analyze_one_evaluator`, assigns severity, can reject+retry), `voting_rounds>1` majority voting (`_majority_verdict`), a second-opinion `validator` model (`validator.py`, Gemini/Vertex), and `--claude-verification` (Claude sonnet cross-check, writes `claude_*` fields).
